...
## shannon rate_____________________________________________________________________________
    sat_percent = 90
    network_shannon_rate = Network('../Resources/263697.json', 'shannon')
    n_node = len(network_shannon_rate.nodes.keys())
    saturationflex = []
    Msflex = []
    M = 1
    while (1):
        ...
        for i in range(n_node):
            t_mtx[i][i] = 0
        elements = list(itertools.permutations(network_shannon_rate.nodes.keys(), 2))
        n_elem = len(elements)
        for e in elements:  # remove the diagonal
            if e[0] == e[1]:
                elements.remove(e)
        for i in range(120):
            if len(elements) == 0:
                break
            el = random.choice(elements)
            val = network_shannon_rate.update_traffic_matrix(t_mtx, el[0], el[1])
            if (val < 0) | (val == np.inf):
                elements.remove(el)
        sat = 0
        for row in t_mtx:
        ...
        if sat > sat_percent:
            break
        M += 1
        network_shannon_rate.free_space()
    plt.plot(Msflex, saturationflex)
    ...
    plt.grid(linestyle='-', linewidth=0.5)
    plt.draw()
    plt.savefig('../Results/Shannon_rate_2.png')
